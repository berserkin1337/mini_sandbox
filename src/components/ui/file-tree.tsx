...
const fileIconExtensions = ['css', 'html', 'js', 'json', 'md', 'ts'] as const

type FileIconExtension = (typeof fileIconExtensions)[number]

const fileIconThemes = {
  css: { color: '#42a5f5', label: '#' },
  html: { color: '#e44d26', label: '<>' },
  js: { color: '#f1e05a', label: 'JS' },
  json: { color: '#f1e05a', label: '{}' },
  md: { color: '#519aba', label: 'M' },
  ts: { color: '#3178c6', label: 'TS' },
} satisfies Record<FileIconExtension, { color: string; label: string }>

function isFileIconExtension(
  extension: string,
): extension is FileIconExtension {
  return fileIconExtensions.includes(extension as FileIconExtension)
}
...
function ReactFileIcon() {
  return (
    <svg
      aria-hidden="true"
      className="size-4 shrink-0 text-[#61dafb]"
...
function FileTypeIcon({ fileName }: { fileName: string }) {
  ...
  if (!isFileIconExtension(extension)) {
    return <Codicon className="text-[var(--github-fg-muted)]" name="file" />
  }

  const iconTheme = fileIconThemes[extension]

  return (
    <span
      aria-hidden="true"
      className="inline-flex size-4 shrink-0 items-center justify-center rounded-[3px] text-[9px] leading-none font-semibold"
      style={{
        backgroundColor: `${iconTheme.color}26`,
      ...
      >
        ...
        >
          {expandedItems?.includes(value)
            ? (openIcon ?? (
                <Codicon className="text-[#dcb67a]" name="folder-opened" />
              ))
            : (closeIcon ?? (
                <Codicon className="text-[#dcb67a]" name="folder" />
              ))}
          <span>{element}</span>
        </AccordionPrimitive.Trigger>
  ...
  (
    ...
    return (
      <button
        ...
        {...props}
      >
        {fileIcon ?? <FileTypeIcon fileName={fileName ?? value} />}
        {children}
      </button>
    )
  },
